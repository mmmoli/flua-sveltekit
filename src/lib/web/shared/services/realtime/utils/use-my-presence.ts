...
type PresenceStore = Pick<Writable<Presence>, 'subscribe'> & {
	update: (newPresence: Partial<Presence>) => void;
};

export function useMyPresence(): PresenceStore {
	const room = useRoom();
	const { subscribe, set } = writable<Presence>();

	function update(newPresence: Partial<Presence>) {
